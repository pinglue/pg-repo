...
import { print, style, commanderHelp as help } from "@pinglue/print/node";
...
import dumpRegistryAction from "./actions/dump-registry.js";
...
program
  ...
  .description(
    "Show info about packages. If route provided then the info will be about that route, otherwise it'll be about general info about all routes"
  )
  .option(
    "-e, --env <env-type>",
    "Specifying the environment for the hub (defaults to local)"
  )
  .option(
    "-p, --profiles <profiles>",
    "Specifying the profiles for the hub (defaults to dev)"
  )
...
program
  ...
  .action(exportAction({ style, print }));

program
  .command("env-temp")
  .description(
    "Create a template env file based on plugins env info (found in pg.yaml)"
  )
  .option("-n, --name <filename>", "Name of the environment. Default: temp")
  .option(
    "-s, --show",
    "Just show the env template file on screen without writing it into the disk."
  )
  .action(envTempAction({ style, print }));

program
  .command("channels [route-name]")
  .description(
    "Show a realtime channel report for the given route (defaults to /) by creating a hub and show its channel report"
  )
  .option(
    "-e, --env <env-type>",
    "Specifying the environment for the hub (defaults to local)"
  ...
  .action(channelsAction({ style, print }));

program
  .command("channels-man [route-name]")
  .description(
    "Generates channel manual for the given route (defaults to /) based on yaml files in the channels folder of the installed pg modules"
  )
  .option(
    "-k, --pkg <package-name>",
    "Filter channels by the registering package"
  )
  .option("-c, --channel <channel-name>", "Filter by the channel name")
  .option(
    "-f, --full",
    "Show the full report for each channel (including description, params, return, etc.)"
  )
  .option(
    "-e, --env <env-type>",
    "Specifying the environment for the hub (defaults to local)"
  ...
  .action(channelsManAction({ style, print }));
...
program
  ...
  .option(
    "-k, --pkg <package-name>",
    "Show the registry record only for this package"
  )
  ...
  .action(dumpRegistryAction({ style, print }));
